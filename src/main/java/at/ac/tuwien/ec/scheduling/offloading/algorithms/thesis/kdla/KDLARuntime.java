...
public class KDLARuntime extends BaseKDLA {
  ...
  protected double calcEbl(MobileSoftwareComponent msc, ComputationalNode node, int k) {
    double w_computation_cost = msc.getLocalRuntimeOnNode(node, this.currentInfrastructure);
    double c_avg_communication_cost = CalcUtils.calcAverageCommunicationCost(msc, node, this.currentInfrastructure);
